...
const EFI_GUID loader_guid = {
	0xdc578480, 0x65d4, 0x4d38,
	{ 0x81, 0x7a, 0x88, 0xf7, 0xaf, 0xc7, 0x56, 0x19 }
};
			  ...
			  const VOID *buf, UINTN size, BOOLEAN persistent)
{
	...
	return uefi_call_wrapper(RT->SetVariable, 5, (CHAR16 *) name,
				 (EFI_GUID *)vendor, flags, size, (VOID *) buf);
}

EFI_STATUS efivar_set(const CHAR16 *name, const CHAR16 *value,
		      BOOLEAN persistent)
{
	return efivar_set_raw(&loader_guid, name, value,
			  ...
			  CHAR8 **buffer, UINTN *size)
{
	...
	err = uefi_call_wrapper(RT->GetVariable, 5,
				(CHAR16 *) name, (EFI_GUID *)vendor,
				NULL, &l, buf);
	if (!EFI_ERROR(err)) {
		     ...
		     UINTN size, CHAR8 **ret, UINTN *ret_size)
{
	...
	err = uefi_call_wrapper(dir->Open, 5, dir, &handle, (CHAR16 *) name,
