				     ...
				     UINTN *addrs, UINTN *offsets, UINTN *sizes)
{
	...
	offset = dos->ExeHeader + sizeof(*pe) +
	...
	for (i = 0; i < pe->FileHeader.NumberOfSections; i++) {
		...
		for (j = 0; sections[j]; j++) {
			if (CompareMem(sect->Name,
				       sections[j], strlena(sections[j])) != 0)
				continue;
				   ...
				   UINTN *addrs, UINTN *offsets, UINTN *sizes)
{
	...
	err = uefi_call_wrapper(dir->Open, 5,
				dir, &handle, path, EFI_FILE_MODE_READ, 0ULL);
	if (EFI_ERROR(err))
		return err;
	...
	headerlen = sizeof(dos) + sizeof(pe) +
		    pe.FileHeader.SizeOfOptionalHeader +
	...
	err = pe_memory_locate_sections(header,
